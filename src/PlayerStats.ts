...
/**
 * Interfaz que define la estructura de datos de un jugador persistente.
 * Se guarda/carga del localStorage en formato JSON.
 */
export interface PlayerData {
  id: string;           // nombre único del jugador (ej: "Jugador 1")
  balance: number;      // saldo actual (puede ser > 0 si ganó, < 0 si perdió)
  totalWins: number;    // número total de manos ganadas
  totalLosses: number;  // número total de manos perdidas
  totalRounds: number;  // número total de rondas jugadas
  initialBalance: number; // saldo con el que empezó la sesión (para calcular ganancia/pérdida)
}
...
export class PlayerStatsManager {
  private storageKey = "blackjack_players"; // clave para localStorage
